...
const FIRST_PARTY_IMAGE = /ghcr\.io\/studio-ch\/rakazo-xcloud(?:-(updater|computer))?(?=[}:@]|$)/g;

function firstPartyImageNames(value: unknown): string[] {
  if (typeof value !== "string") return [];
  return [...value.matchAll(FIRST_PARTY_IMAGE)].map((match) => match[1] ?? "app");
}
...
describe("the images compose file", () => {
  it("runs postgres, app roles, supervisor, and a published computer image", () => {
    ...
    expect(compose.services.computer?.image).toContain("ghcr.io/studio-ch/rakazo-xcloud-computer");
  ...
  it("skips non-string Compose environment scalars when collecting image names", () => {
    ...
    expect(firstPartyImageNames("ghcr.io/studio-ch/rakazo-xcloud-computer:edge")).toEqual([
      "computer",
    ]);
  });
